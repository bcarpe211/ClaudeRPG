...
Shipped in 0.4.9. Supported re-enrollment, recoverable removal, complete local
removal, readable status, and `raiders status --json` compatibility are recorded
in the active [companion operations](runtime-raiders/companion-operations.md)
and [employee beta](runtime-raiders/employee-beta.md) procedures. The detailed
completed work order is preserved with the executed companion UX and
re-enrollment archive; use it as historical evidence only.
