...
1. Implement and test the publication mechanism, freeze the final release SHA,
   rebuild its signed triplet, and finish pre-cutover validation with no Runtime
...
5. Run the final read-only preflight and require all three artifact URLs to
   remain `404`.
...
8. Separately authorize publication of the exact signed companion triplet and
   independently verify all three HTTPS digests.
9. Separately authorize installation on one canary and prove its daemon is live
   but collection is persistently off.
10. Separately authorize live canary activation and complete the Run checks.
11. Separately authorize office activation only after canary acceptance.
...
- [ ] Freeze the final release SHA, rebuild/re-notarize its companion triplet,
      and record all three final digest values.
...
- [ ] Separately authorize Caddy preparation; record the prior Caddy config
      ...
      responses and three artifact `404` responses, and leave `current` absent.
...
- [ ] Separately authorize publication of only the recorded signed triplet.
- [ ] Run `scripts/pi/runtime-raiders-artifacts.sh publish` with the exact release
      SHA and installer, ZIP, and checksum-file digests, then run
      `scripts/pi/runtime-raiders-artifacts.sh status`.
- [ ] Independently download and verify all three HTTPS responses, require
...
- [ ] Separately authorize installation on one canary; use only the locally
...
Require `status` to report `unpublished`, all three artifact URLs to return
...
- final rendered-installer, signed-ZIP, and checksum-file SHA-256 values;
...
> remain absent; both internal health URLs must return `200`; and all three artifact URLs must return `404`:
...
> companion triplet for release SHA `________________________________________`
> with recorded installer, ZIP, and checksum-file SHA-256 values
...
> After three-digest publication acceptance, I authorize independently
> downloading and installing the recorded triplet on one approved canary. The
...
> After installed-off acceptance, I authorize `raiders on` for the single
> recorded canary and the content-free Codex Desktop/CLI acceptance checks. This
> does not authorize office installation or activation.

### H. Office activation

> After live-canary acceptance, I authorize participating office users to
