...
function protocolTwoArtifact(root: string): { zip: string; checksum: string } {
  ...
  executable(join(agent, 'Contents/MacOS/runtime-raiders-agent'), [
    ...
    '  __runtime-raiders-installer-sync-migration) case "${2:-}" in staging-tree|staging-tombstone-tree|active-journal|support-directory) exit 0;; *) exit 64;; esac;;',
...
describe('Runtime Raiders protocol-two installer', () => {
  ...
  it('recovers and retries after actual SIGKILL at every durable migration boundary', () => {
    const boundaries = [
      ...
      'command-link-replacement', 'bootstrap', 'prepared-health',
      'acceptance-mark',
    ];
  ...
  it.each([
    ...
    try {
      ...
      if (['after-commit-marker', 'after-candidate-resume',
