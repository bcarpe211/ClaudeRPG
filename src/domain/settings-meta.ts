...
export const SETTINGS_META: Record<string, SettingMeta> = {
  ...
  base_hit: { group: 'Combat', label: 'Base hit damage', unit: 'dmg', min: 1, step: 1,
    description: 'Damage of one swing at level 1 with no Momentum bonus. Higher = everyone hits harder — but monster HP is sized to incoming Raid Power, so this mostly rescales the numbers rather than the pace.' },
  ...
  decay_after_minutes: { group: 'Momentum', label: 'Momentum hold time', unit: 'min', min: 0, step: 1,
    description: "Minutes with no new Raid Power before a Raider's Momentum starts fading. Higher = Momentum lingers longer after the Raider's Raid Power stops rising." },
  ...
  baseline_battle_minutes: { group: 'Monster HP & difficulty', label: 'Baseline Fight length', unit: 'min', min: 1, step: 1,
    description: 'How long a monster lasts at baseline Raid Power with no Momentum bonus. New Raid Power shortens the Fight. Higher = tougher, longer monsters.' },
  ...
  pause_after_minutes: { group: 'Encounters & pacing', label: 'Idle-pause delay', unit: 'min', min: 1, step: 1,
    description: 'Minutes with no new Raid Power anywhere in the current Raid before the game pauses ("the dungeon rests"). Higher = the Raid keeps running through longer lulls.' },
  // Shop
  ...
  potion_gold_t1_duration_s: { group: 'Potions', label: 'Gold Potion (T1) duration', unit: 's', min: 1, step: 60,
    description: 'Combat-active seconds for future Beginner Gold Potion activations. Higher = its Raid Power reward window lasts longer.' },
  potion_gold_t1_gold_per_1000: { group: 'Potions', label: 'Gold Potion reward per 1,000 Raid Power', unit: 'gold', min: 0, step: 1,
    description: 'Gold awarded for each whole 1,000 eligible Raid Power during a future Gold Potion activation. Higher = more gold per Raid Power milestone.' },
  ...
  potion_daily_stock_per_sku: { group: 'Potions', label: 'Daily potion stock per SKU', unit: 'count', min: 0, step: 1,
    description: 'How many units of each future potion SKU a Raider may buy per local game day. Higher = more daily purchases.' },
  potion_daily_uses_per_type: { group: 'Potions', label: 'Daily potion uses per type', unit: 'count', min: 0, step: 1,
    description: 'How many activations of each future potion type a Raider may start per local game day. Higher = more daily uses.' },
  // Reward allocation
